...
## 진행 상황 갱신 (2026-09-19, 이어서 2) — 자동발행 부가세 버그 수정 + 거래처 과세유형

사용자가 "세금계산서-영수증 연동이 완벽하게 원하는 대로 됐는지" 재확인을 요청하면서, 검토 과정에서 실제 버그를 하나 발견하고 고침. `KOP-BACKEND` 커밋 `f274bc5`, `KOP-FRONTEND` 커밋 `a5a9d7e`(둘 다 로컬만).

- **발견한 버그**: `TaxInvoiceAutoIssuanceService`가 영수증 품목 단가에 새로 10%를 더해 세금계산서를 만들고 있었다. `receipt.amount`는 이미 "실제 청구·입금된 총액"인데(부가세 포함 여부와 무관하게 등록자가 실제 합의 금액을 그대로 입력), 그 위에 부가세를 한 번 더 얹은 셈이라 자동발행된 세금계산서 합계가 항상 실제 입금액보다 10% 컸다. 이건 간이과세자 얘기와 별개로 **모든 거래처에 적용되던 계산 버그**.
- **수정**: `TaxInvoiceItem.ofInclusive`/`TaxInvoice.replaceItemsInclusive` 신규 — unitPrice×quantity를 이미 부가세 포함된 최종 수령액으로 보고 거꾸로 공급가액·세액을 나눈다(공급가액 = 총액÷1.1). 자동발행 경로만 이걸 쓰고, 수동 작성(`TaxInvoiceService`)은 기존 그대로(세전 단가 + 10% 가산) — 이건 화면에도 이미 그렇게 문서화돼 있던 맞는 동작이라 안 건드림.
- **거래처 과세유형 추가**: `Client.taxType`(일반과세자/간이과세자/면세사업자/비영리·비사업자, V9 마이그레이션, 기본값 GENERAL). 사용자가 "간이과세자·면세사업자·비영리단체(교회 등)에 납품하면 부가세를 별도로 못 받아서 우리만 손해보는 구조 아니냐"는 질문에서 출발 — 실제로 맞는 얘기(부가세법상 매입세액공제 못 받는 매입자는 부가세를 안 내려는 유인이 있고 강제할 방법이 없음, 우리가 일반과세자면 받은 돈에서 거꾸로 부가세를 계산해 신고해야 해서 그만큼 손해). 다만 최종 계산 로직 자체는 거래처 유형과 무관하게 항상 "영수증의 실제 총액을 역산"하는 동일한 방식이라, `taxType`은 현재는 기록·선택용(향후 확장 여지)이고 계산 분기에는 안 쓴다 — 처음엔 "품목 단가를 세전으로 통일 + 일반과세자만 자동 10% 가산" 설계로 갔다가, 사용자가 "영수증 금액은 원래 부가세 포함이고 세금계산서만 세전 입력받아 포함가를 보여주는 게 기존 로직"이라고 정정해서 되돌림(Receipt 엔티티에 잠깐 추가했던 supplyAmount/vatAmount 필드와 ReceiptService의 ClientRepository 의존성은 전부 롤백).
- **중요 발견 — 영수증 폼에 거래처 연결 UI가 아예 없었음**: `receiptApi.ts`가 등록/수정 시 `clientId`를 항상 `null`로 고정 전송하고 있었고, `ReceiptForm`엔 거래처 선택 필드 자체가 없었다. 이 상태로는 지난 갱신에서 완성했다고 적은 자동 매칭→자동발행 전체 파이프라인이 **프론트에서 절대 트리거될 수 없었음**(항상 clientId=null). 이번에 `ReceiptForm`에 "거래처 연결(선택)" 드롭다운을 추가(`useClients`로 목록 조회, nullable — 자유 입력 vendorName만으로도 계속 등록 가능)하고 `receiptApi.ts`의 하드코딩을 제거해서 실제로 연결되게 고침.
- **검증**: 신규/수정 백엔드 테스트 포함 finance-service 전체 GREEN. V9을 실제 로컬 Postgres에 적용 + `ddl-auto=validate` 통과. 프론트 tsc/vitest(83개) 통과. **실제 브라우저 풀플로우**: 간이과세자 거래처 생성(과세유형 선택 UI 확인) → 영수증 등록 시 그 거래처 연결(드롭다운에 실제로 뜨는 것 확인) → 관리자가 입금 확인 → 세금계산서 자동발행된 것을 목록에서 확인 → 상세 페이지에서 공급가액 ₩18,182 + 세액 ₩1,818 = 합계 ₩20,000(영수증 금액과 정확히 일치) 확인.
