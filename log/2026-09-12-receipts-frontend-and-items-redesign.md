...
## 8. 인쇄 시 페이지 배경·여백까지 찍히던 문제

"영수증만 인쇄되는 게 아니라 페이지 전체가 찍히는 것 아니냐"는 지적 — 맞았다. `print:hidden`으로 버튼·네비게이션은 숨겼지만 `body`의 회색 배경과 페이지 wrapper의 `padding`(`px-6 py-10`)은 그대로 남아 있어서 영수증 슬립 하나만 찍히는 게 아니라 그 여백까지 같이 인쇄됐다. `body` 배경을 인쇄 시 흰색으로, 페이지 wrapper의 padding/max-width를 인쇄 시 0/none으로 되돌리고 `@page { margin: 10mm }`를 지정해 슬립만 깔끔하게 찍히도록 고쳤다.

## 9. STAFF 계정 실제 HTTP·브라우저 테스트

지금까지 "직원은 본인 것만/24시간 이내만" 규칙은 Mockito 단위 테스트로만 검증했었다. 실제로 ADMIN이 STAFF를 초대(`POST /api/auth/invitations`) → 초대 토큰은 로컬 환경이라 이메일 대신 DB(`auth.employee_invitations.token`)에서 직접 조회 → 수락·로그인까지 실제로 거쳐서 검증했다.

**curl로 확인한 것(전부 기대대로 동작)**:
- STAFF 영수증 등록 성공
- STAFF 목록 조회 → 본인 것만 보임(관리자 것 안 섞임)
- STAFF가 본인 영수증 상세 조회 → 200
- STAFF가 관리자 영수증 상세/수정/삭제 시도 → 전부 403 FORBIDDEN
- STAFF가 직원 관리(`GET /api/auth/employees`, ADMIN 전용) 접근 시도 → 403
- STAFF가 등록 직후(24시간 이내) 본인 건 수정 → 200
- ADMIN은 STAFF 영수증 상세 조회·`createdBy` 필터로 특정 직원 것만 조회 가능
- **24시간 초과 케이스**: DB에서 `created_at`을 25시간 전으로 직접 돌린 뒤 — STAFF 수정/삭제 시도 → 403, 같은 건을 ADMIN이 수정 → 200(관리자는 기간 제한 없음) 전부 확인

**브라우저(Playwright MCP)로 확인한 것**: STAFF로 로그인 → 영수증 등록 폼·상세(영수증 슬립) 정상 렌더 → 관리자 소유 영수증 URL에 직접 접근 시 크래시 없이 "영수증을 찾을 수 없습니다"로 안전하게 처리(403을 세분화해서 노출하지 않음).

**자동화 E2E는 안 만듦**: STAFF 초대 수락에 필요한 토큰이 이메일로만 나가고(로컬은 DB 직접 조회로 대신함) 프론트 화면에 노출되는 값이 아니라서, `e2e/receipts.spec.ts`처럼 브라우저만으로 자기완결적인 자동화 테스트를 만들려면 테스트 코드에 DB 접근 권한을 새로 열어줘야 한다 — 이번 라운드에서는 안 했고, 수동(curl+MCP) 검증으로 대신했다.
...
- 이 과정에서 실제 브라우저 동작(새로고침 세션 복구, 타임존, null 파라미터 캐스팅, 네이티브 폼 검증 충돌, 인쇄 시 페이지 여백)에서만 드러나는 버그 6건을 잡았다 — 전부 유닛 테스트만으로는 못 잡고 curl/브라우저 직접 조작으로 발견
- 영수증 E2E 자동화 테스트(8개)를 추가해 앞으로는 회귀로 남는다. 백엔드 테스트도 케이스별 로그가 콘솔에 바로 보이도록 설정 변경
- STAFF 계정을 실제로 만들어 "본인 것만 조회·24시간 이내만 수정" 규칙을 curl·브라우저로 전부 재확인 — 단위 테스트와 실제 동작이 일치함을 확인
